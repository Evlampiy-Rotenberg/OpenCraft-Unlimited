...
namespace OpenCraft
{
    ...
    {
        private readonly Dictionary<(int, int, int), Chunk> World;

        public VoxelRaycaster(Dictionary<(int, int, int), Chunk> world)
        {
            World = world;
        }
        ...
        public bool IsSolidBlockWorld(int wx, int wy, int wz)
        {
            ...
            if (!World.TryGetValue((cx, cy, cz), out Chunk? chunk))
                return false;
        ...
        public bool SetBlockWorld(int wx, int wy, int wz, byte id)
        {
            ...
            if (lx == 0)
            {
                if (World.TryGetValue((cx - 1, cy, cz), out Chunk? left)) left.GenerateMesh();
            }
            else if (lx == S - 1)
            {
                if (World.TryGetValue((cx + 1, cy, cz), out Chunk? right)) right.GenerateMesh();
            }

            // Y
            if (ly == 0)
            {
                if (World.TryGetValue((cx, cy - 1, cz), out Chunk? down)) down.GenerateMesh();
            }
            else if (ly == S - 1)
            {
                if (World.TryGetValue((cx, cy + 1, cz), out Chunk? up)) up.GenerateMesh();
            }

            // Z
            if (lz == 0)
            {
                if (World.TryGetValue((cx, cy, cz - 1), out Chunk? back)) back.GenerateMesh();
            }
            else if (lz == S - 1)
            {
                if (World.TryGetValue((cx, cy, cz + 1), out Chunk? front)) front.GenerateMesh();
            }
