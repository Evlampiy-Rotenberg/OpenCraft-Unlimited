namespace OpenCraft
{
    public class Chunk
    {
        ...
        public Chunk(int x, int y, int z, SimpleGridAtlas atlas, Dictionary<(int,int,int), Chunk> chunks, int seed)
        {
            Atlas = atlas;
            X = x;
            Y = y;
            ...
            for (int x = 0; x < Size; x++)
            {
                for (int y = 0; y < Size; y++)
                {
                    for (int z = 0; z < Size; z++)
                    {
                        ...
                        byte blockType = GenerateBlock(blockX, blockY, blockZ);
        ...
        int GetBlockId(int blockX, int blockY, int blockZ, int surfaceHeight)
        {
            if (blockY > surfaceHeight)
                return AIR_ID;

            int a = 8;
            int randY = blockY + random.Next(-a, a+1);
        ...
        private byte GenerateBlock(int blockX, int blockY, int blockZ)
        {
            const int heightScale = 512;

            float n = noise.GetNoise(blockX, blockZ);

            float mountainsPower = 3.0f;
            float valleysPower = 3.0f;
            float valleyScale = 0.25f;
