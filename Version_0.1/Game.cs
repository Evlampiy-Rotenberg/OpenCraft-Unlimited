...
namespace OpenCraft
{
    internal class Game: GameWindow
    {
        ...
        GameObject player = new GameObject(new Vector3(0, 8, 0), new Vector3(0.8f, 1.8f, 0.8f));
        ...
        protected override void OnUpdateFrame(FrameEventArgs args)
        {
            ...
            if (_raycaster.RaycastBlock(origin, dir, reach, out var hit, out var normal))
            {
                ...
                if (MouseState.IsButtonPressed(MouseButton.Right))
                {
                    ...
                    if (!touched.Contains(placePos))
                        _raycaster.SetBlockWorld(placePos, 6);
                }
            ...
            foreach (var (dx, dy, dz) in defaultOffsets)
            {
                ...
                {
                    World[key] = new Chunk(key.x, key.y, key.z, atlas, World);
                    chunksGenerated++;
                }
        ...
        private void LoadCrosshair()
        {
            ...
            float size = 0.025f; // розмір прицілу в NDC

            float[] crossVertices =
            {
                -size,  0f,   size,  0f,   // горизонтальна лінія
                 0f,  -size,  0f,  size    // вертикальна лінія
            };
        ...
        private void RenderCrosshair()
        {
            ...
            crosshairShader.Use();   // простий шейдер без матриць
